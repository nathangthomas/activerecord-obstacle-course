...
# ----------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      items_for_user_3_third_order = names = Order.joins(:items).joins(:order_items).where("order_items.order_id =?", @order_9.id).pluck("items.name").uniq
      #How would I do this if I did not know the order number?
    # ------------------------------------------------------------
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    average = Order.joins(:user).where("orders.user_id = ?", @user_3.id).average(:amount).to_i
